...
`.github/workflows/ci.yml` lance automatiquement, à chaque push/pull
request : `flutter pub get` → génération `drift` → `flutter analyze` →
`flutter test`, plus deux builds best-effort (Android APK debug, Web) en
`continue-on-error` — ils ne bloquent pas le workflow s'ils échouent,
puisque plusieurs points sont documentés comme non vérifiés (voir
`docs/ecclesia-etape10-multiplateforme.md`). Les dossiers de plateforme
(`android/`, `web/`) n'existent pas dans ce dépôt — le workflow les
génère à la volée via `flutter create .` s'ils sont absents.
